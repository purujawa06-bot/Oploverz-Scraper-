...
export const StreamPage: React.FC = () => {
    ...
    const fetchStream = async () => {
      ...
      if (url) {
        try {
          ...
          setData(result);
          document.title = `Watch ${result.title || 'Anime'} - NimeStream`;
  ...
  if (!data) return <div className="text-center p-20 text-white">Stream not available.</div>;
        ...
        <div className="md:col-span-2 space-y-6">
          {/* Stream Selectors */}
           <div className="bg-slate-900 rounded-lg p-4 border border-slate-800">
             <h3 className="text-sm font-bold text-slate-400 uppercase mb-3 tracking-wider">Stream Quality</h3>
             <div className="flex flex-wrap gap-2">
                ...
                {(!data.stream_links || data.stream_links.length === 0) && (
                  <div className="text-slate-500 text-sm">No stream links available.</div>
                )}
             </div>
           </div>
           ...
           <div className="flex justify-between items-center">
             {data.navigation?.prev ? (
                ...
                >
                  <ChevronLeft size={18} /> Previous Episode
                </Link>
             ) : (
               <div /> // Spacer
             )}
             ...
             {data.navigation?.next && (
                ...
                >
                  Next Episode <ChevronRight size={18} />
                </Link>
             )}
           </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-slate-900 rounded-xl p-5 border border-slate-800">
             <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
              <Download className="text-indigo-500" size={20} /> Downloads
             </h3>
             <div className="space-y-4">
